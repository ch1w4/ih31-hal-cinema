...
---
## ollamaのインストール
ollamaは、ローカルでAIモデルを実行するためのツールです。以下の手順でインストールしてください。
1. **ollamaのダウンロード**  
   [ollama公式サイト](https://ollama.com/)から、対応するOSのインストーラーをダウンロードしてインストールしてください。
2. **ベースモデルのダウンロード**
    ターミナルで以下のコマンドを実行して、phi3:miniモデルをダウンロードします。
    ```bash
    ollama pull phi3:mini
    ```
    > ⚠️ 学校のネットワークではタイムアウトする場合があります。その場合は自宅などで実行してください。

3. **movie-recモデルの作成**
    `backend/` ディレクトリで以下のコマンドを実行して、カスタムモデルを作成します。
    ```bash
    ollama create movie-rec -f Modelfile
    ```
    モデルを更新する場合は一度削除してから再作成してください。
    ```bash
    ollama rm movie-rec
    ollama create movie-rec -f Modelfile
    ```
...
4. **モデルの起動**
    裏でollamaのモデルを起動するため、以下のコマンドを実行してください。
    ```bash
    ollama run movie-rec
    ```
    起動させないとAIシステムがモデルにアクセスできないため、必ず起動させてください。
...
```
モデルを更新した場合は `ollama rm movie-rec` で削除後、`ollama create movie-rec -f Modelfile` で再作成してください。
